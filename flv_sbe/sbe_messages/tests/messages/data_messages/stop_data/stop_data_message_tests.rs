...
fn test_new() {
    ...
    assert_eq!(message.exchange_id(), &1);
...
fn test_encode() {
    ...
    let expected: Vec<u8> = vec![7, 0, 202, 0, 1, 0, 1, 0, 202, 0, 1, 0, 1, 1, 0];
    let actual = buffer;
...
fn test_decode() {
    let symbol_id = SymbolID::BTCUSD as u16;
    let encoded: Vec<u8> = vec![7, 0, 202, 0, 1, 0, 1, 0, 202, 0, 1, 0, 1, 1, 0];
...
fn test_display() {
    ...
    let expected =
        "StopDataMessage[message_type: StopData, client_id: 1, exchange_id: 1, symbol_id: 1]";
